...
#include "Rule_34_35.h"
Point Get_from_l1(double p4, double u4, Point C2, Point C1, START start)
{
	double u5 = 0;
	double p5 = 0;
	...
	if (C2.p <= p4 && p4 < C1.p && C2.u <= u4 && u4 < C1.u)
	{
		...
		u5 = U5(u4, a1, delta, start.gamma1, alpha);
		p5 = P5(p4, u4, u5, ro4);
	}
...
TwoPoints Search_Conf_A1(START &start, Point &C2, Point &C1)
{
	...
	while (true)//start down from (p1,u1)
	{
		...
		pl1 = Get_from_l1(p, u, C2, C1, start);//point from l1
...
bool Check_RES_Inflow(Point N_L2, Point N_L1)
{
	double p4 = N_L2.p;
	double u4 = N_L2.u;
	double p5 = N_L1.p;
